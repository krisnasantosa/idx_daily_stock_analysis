...
MARKET_EXCHANGE = {"cn": "XSHG", "hk": "XHKG", "us": "XNYS", "idx": "XIDX"}
...
def get_market_for_stock(code: str) -> Optional[str]:
    ...
    Returns:
        'cn' | 'hk' | 'us' | 'idx' | None (None = unrecognized, fail-open: treat as open)
    """
    if not code or not isinstance(code, str):
        return None
    code = (code or "").strip().upper()

    from data_provider import is_us_stock_code, is_us_index_code, is_hk_stock_code, is_idx_stock_code
...
def get_open_markets_today() -> Set[str]:
    ...
    Returns:
        Set of market keys ('cn', 'hk', 'us', 'idx') that are trading today
    """
    if not _XCALS_AVAILABLE:
        return {"cn", "hk", "us", "idx"}
    result: Set[str] = set()
